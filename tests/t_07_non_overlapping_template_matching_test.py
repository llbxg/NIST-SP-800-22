...
def non_overlapping_template_matching_test(key, n, m=9, B=None, N=8, b_print=True):
    if n <1000000:
        __print (b_print, '{:40} : Error. Need at  1,000,000 bits. Got {}.'.format('non overlapping template matching test',n))
        return [0], False
    ...
    if M <= 0.01*n:
        __print (b_print, '{:40} : Error. M is NOT selected correctly.'.format('non overlapping template matching test'))
        return [0], False
    ...
    __print(b_print, '{:40} : {:.3f} -> {} '.format('non overlapping template matching test',p,b))
